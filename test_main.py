...
def test_delete_nonexistent_record():
    ...
    assert response.status_code in (404, 500)  # Either 404 Not Found or 500 Internal Server Error is acceptable, depending on the database implementation.
    assert "detail" in response.json()  # There should be a detail message, depending on the database implementation.
